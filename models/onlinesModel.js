...
export const addUser = async (userId) => {
    await pool.query('INSERT INTO onlines (user_id, online, last_activity_time) VALUES ($1, $2, $3)', [userId, false, Date.now()]);
}

// it also updates last_activity_time to Date.now()
export const updateOnlineStatus = async (userId, isOnline) => {
    const result = await pool.query('UPDATE onlines SET online = $2, last_activity_time = $3 WHERE user_id = $1 RETURNING *', [userId, isOnline, Date.now()]);
    if (result.rows[0])
